...
**Why:** Phase 6 (dashboard wiring) and Phase 8 (chat sessions) both started this way. When a prior session got cut off mid-build, the plan doc let the next session pick up cleanly. Without it, each restart is a re-discovery exercise.

**How to apply:**
- File: `PLAN-<FEATURE-NAME>.md` at repo root (e.g. `PLAN-DASHBOARD-WIRING.md`).
...
- Subsequent commits use phase-numbered prefixes: `(Phase 8a)`, `(Phase 8b/2)`, etc.
- Add a `## Source of truth (resume from here if interrupted)` section listing exact mcp tool calls a new session should run.
